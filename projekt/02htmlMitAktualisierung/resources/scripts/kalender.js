let kalender = {
  ...
  get wochenstart()  { let datum  = this.heute;
                       let datum0 = datum.setTime(
                                            datum.getTime()
                                            - (this.wochentag.nr-1)
                                               *1000*60*60*24);
                           datum0 = new Date(datum0);
                       return { datum : datum0,
                                tag   : datum0.getDate(),
                                monat : datum0.getMonth()+1,
                                monatName : this._monate[datum0.getMonth()],
                                jahr  : datum0.getFullYear()
                              };
                     },
  ...
  get wochenende()   { let datumEnde = this.wochenstart
                                           .datum
                                           .getTime()
                                           + 5*1000*60*60*24;
                           datumEnde = new Date(datumEnde);
                        return { datum     : datumEnde,
                                 tag       : datumEnde.getDate(),
                                 monat     : datumEnde.getMonth()+1,
                                 monatName : this._monate[datumEnde.getMonth()],
                                 jahr      : datumEnde.getFullYear()
                              };
                      },
